...
export default {
	...
	async mounted() {
		try {
			...
			const apkAsset = data.assets.find((a) => /^ireader-v\d+\.\d+\.\d+.apk/.test(a.name));
		...
		try {
			...
			const apkAsset = data.assets.find((a) => /^ireader-r\d{4,}.apk/.test(a.name));
	...
	methods: {
		downloadStable() {
			this.$swal({
				title: "Downloading",
				html: `Started downloading <strong>IReader Stable</strong>`,
				icon: "success",
				focusConfirm: false,
				focusCancel: false,
				timer: 3000,
			...
			window.ga("send", "event", "Action", "Download", "IReader");
		},
		downloadPreview() {
			this.$swal({
				icon: "warning",
				title: "Are you sure?",
				html: `
					<strong>IReader Preview</strong> is not recommended if you're not willing to test for – and endure – issues.
					<div class="note">
						<p>
							Read more about IReader Preview
							<a href="../help/faq/#what-is-ireader-preview" rel="help">here</a>.
						</p>
					</div>`,
				...
				if (result.value) {
					this.$swal({
						icon: "success",
						title: "Downloading",
						html: `Started downloading <strong>IReader Preview</strong>`,
					...
					window.ga("send", "event", "Action", "Download", "IReader Preview");
				}
			});
		},
	},
